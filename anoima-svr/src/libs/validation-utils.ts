...
function notFound<T>(value: T, name?: string): T {
	...
	if (!value) {
		throw new HttpError(404, name ? makeMessage(undefined, name, "is not found") : undefined);
	}
	return value;
}
...
function toNumber(value: any, name?: string): number {
	...
	value = Number(value);
	if (isNaN(value)) {
		throw new HttpError(400, makeMessage(value, name, "is not number"));
	}
	return value;
}
...
function min(value: any, min: number, name?: string): number {
	...
	if (value < min) {
		throw new HttpError(400, makeMessage(value, name, " < " + min));
	}
	return value;
}
...
function max(value: any, max: number, name?: string): number {
	value = toNumber(value);
	if (value > max) {
		throw new HttpError(400, makeMessage(value, name, " > " + max));
	}
	return value;
}
